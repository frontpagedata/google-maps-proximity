...
date: "Last updated on `r Sys.Date()` - Data can be found [here](https://docs.google.com/spreadsheets/d/1uOKfBcs_RgAxRf8fAlhmH8f5VLzPtf1t7enPWVT1mnM/edit?usp=sharing)"
output:
  html_document:
    theme: paper
    highlight: kate
...
```{r fig.align='center', fig.height=6, out.width='70%'}
...
ten_miles_samples <- as.character(seq(11,30))
# see the study plan: 
# https://docs.google.com/spreadsheets/d/1KCDf10VUDOEMKvIobbgmAJub0w1ETslf4oVmrzS8wQQ/

# This might be useful if want to viz both 5-mile and 10-mile samples together:
#   https://github.com/tidyverse/ggplot2/issues/2103
...
to_plot_heatmap <- samples %>% select(City, Sample, rank, Latitude, Longitude) %>%
  ...
  group_by(Sample) %>%
  mutate( # for now, we ttreat them the same, then just show the 10-mile radius samples
...
ggplot(data = to_plot_heatmap,
       aes(x = Long_norm, y = Lat_norm, fill = rank, label = rank_label) ) +
  facet_wrap(vars(Sample), ncol = 5)  +
  geom_tile(show.legend = FALSE) +
  # geom_text() +
  # scale_fill_gradient(low = "#8a0028", high = "#fa9fb5", na.value = "#fde0dd", guide = FALSE) +
  # scale_fill_gradient(low = "#9A3A98", high = "#D4EAFF", na.value = "#cccccc", guide = FALSE) +
  # scale_fill_gradient(low = r_col, high = "#D4EAFF", na.value = "#e6e6e6", guide = FALSE) +
  # scale_fill_gradient(low = "#A90774", high = "#FBE2AF", na.value = "#D3E7F1", guide = FALSE) +
  # scale_fill_gradient(low = "#3E74A5", high = "#F6FBE1", na.value = "#e6e6e6", guide = FALSE) +
  scale_fill_gradient(low = "#005470", high = "#F3E5C9", na.value = "#ff8989", guide = FALSE) +
  coord_equal() + # force square tiles
  theme_void() +
  theme(
  ...
  labs(
    title = "All 20 Miami samples collected with 10-mile radius"
    # caption = "The sample 12 is the one shown above.\nWe observe that the law firm of sample 11 keeps ranking high even at high distance,\nwhereas the sample law firm 13 directly drops out of the top 20 outside its own location."
  )
...
avg_by_city_and_firm %>% 
  ...
  labs(
    title = "Local SERP Position of Law Firms at Their Own Location"
  ) +
...
avg_drop_for_all_cities_widget <- girafe(ggobj = avg_drop_for_all_cities,
                                         options = list(
                                           opts_hover_inv(css = "opacity:0.1;"),
                                           opts_hover(css = "stroke-width:2;")
                                         )
)
avg_drop_for_all_cities_widget


# save the data of the plot in wide format to recreate it in DataWrapper
# (in DataWrapper, each line must be a column):
# we need to interpolate manually for Datawrapper, on the contrary to ggplot2.
# (there are some miles with missing data)
skeletton <- tibble(rough_geodist = as.double(rep(seq(0, 16),50))) %>%
  arrange(rough_geodist) %>%
  mutate(City = rep(avg_by_city$City %>% levels(), 17)) %>%
  arrange(City,rough_geodist )
drop_datawrapper <- skeletton %>%
  left_join(avg_by_city %>% select(City, rough_geodist, avg_drop), 
            by = c("rough_geodist", "City")) %>%
  arrange(City, rough_geodist) %>% 
  mutate(avg_drop = zoo::na.approx(avg_drop, 
                                   na.rm = FALSE,
                                   maxgap = 1) # to not interpolate trailing 0s
  ) %>%
  pivot_wider(names_from = City, values_from = avg_drop) %>% 
  arrange(rough_geodist) 
drop_datawrapper$Minneapolis[17] <- NA # manual correaction (the only one needed because only city without trailing maxgap >= 2)
rio::export(drop_datawrapper, here::here("proc_data", 'drop_datawrapper_data.csv'))
...
# save the data to reproduce this in DataWrapper (not faceted, hover over style):
# data need to be in wide format (in DataWrapper, each line must be a column):

# skeletton was defined in the chunk where the average drop is ploted with ggirafe
out20_datawrapper <- skeletton %>%
  left_join(avg_by_city %>% select(City, rough_geodist, percentage_out_of_top20), 
            by = c("rough_geodist", "City")) %>%
  arrange(City, rough_geodist) %>% 
  mutate(percentage_out_of_top20 = zoo::na.approx(percentage_out_of_top20, 
                                                  na.rm = FALSE,
                                                  maxgap = 1) # to not interpolate trailing 0s
  ) %>%
  mutate(
    percentage_out_of_top20 = round(100*percentage_out_of_top20,1)
  ) %>%
  pivot_wider(names_from = City, values_from = percentage_out_of_top20) %>% 
  arrange(rough_geodist) 
out20_datawrapper$Minneapolis[17] <- NA # manual correction (the only one needed because only city without trailing maxgap >= 2)
rio::export(out20_datawrapper, here::here("proc_data", 'out20_datawrapper_data.csv'))
