...
rank_all_samples_2cities <- ggplot(data = two_cities, 
  ...
  scale_y_reverse(breaks = c(1,5,10,15,20,21)) +
  scale_x_continuous(
...
We observe that the patterns are slightly different between cities. There is nevertheless a consistency: the drop in ranking varies greatly between law firms. Some law firms do only see a small drop in their ranking, even at 5 or 10 miles from their location. Other law firm quickly drop out of the top 20 (showed as 21 on the plot).    
...
The average rank across all law firms is shown in pink. It is computed with a rank of 21 for the firms out of the top 20 and for which the rank is not recorded anymore by Local Falcon. This is distorting the "true" average, which is likely lower at large miles.    

We see that the shape of the average rank by mile is similar between cities: it drops fast in the first mile, and then slowly stabilizes.   

```{r, fig.align='center', fig.height=20, out.width='100%', include=FALSE}

# # geom_smooth LOG
# 
# samples_without_21 <- samples %>%
#   mutate(
#     # rank = ifelse(rank == 21, NA, rank),
#     low_res_dist = round(geodist_miles,1)) %>%
#   group_by(City, Sample, low_res_dist) %>%
#   summarise(avg_rank = mean(rank)) %>%
#   mutate( # to help with log(0) = -Inf
#     low_res_dist = ifelse(low_res_dist == 0, 0.001, low_res_dist)
#   ) %>%
#   mutate(avg_rank = ifelse(avg_rank == 21, 21, avg_rank))
# 
# 
# ggplot(data = samples_without_21 %>% filter(City == "New-York")) +
#   geom_line(mapping = aes(x = low_res_dist, y = avg_rank, group = Sample), 
#             alpha = 0.4) +
#   geom_point(mapping = aes(x = low_res_dist, y = avg_rank, group = Sample)) +
#   geom_smooth(mapping = aes(x = low_res_dist, y = avg_rank),
#               method="glm", formula = y ~ I(log(x)),
#               se=FALSE, size=1, color = r_col) +
#   scale_y_reverse() +
#   labs(
#     title = "testing geom_smooth exponential decay (log)")
...
#                                    mapping = aes(x = rough_geodist, y = avg_rank, group = C(City, Sample))) +
#   geom_line(alpha = 0.4) +
...
In order to better compare the drop in ranking between law firms and cities, we can visualize their drop from their initial position - the relative ranking.  
...
Note that, again, the average is computed with a constant value of "21" for the samples out of the top 20. Nevertheless, all cities see a drop of -5 to -12 in the average ranking of the law firms in the first mile. The drop seems to be larger in the Queens than in New-Orleans.    
...
As we just saw, the drop in term of ranking has the same shape in all cities. The drop seems to follow more or less a rule of [exponential decay](https://en.wikipedia.org/wiki/Exponential_decay): it decreases at a rate proportional to its current value. At first it decreases fast and then reach stability.  

We start by illustrating the fit with all the samples taken in all cities, together. In order to get a better estimate of the exponential decay function, we average the data each tenth of a mile. In pink, we see the average drop in ranking regardless of the city:  
...
Google Maps shows 20 results on the first search page and Local Falcon does not collect the ranking above the topø 20. We saw above that the ranking was dropping fast in the first mile, but also that not all the firms were dropping out of the top 20 after 10 miles. And this, in all cities, regardless of their area.  
