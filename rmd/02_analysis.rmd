...
default_example <- ggplot(data = mpg, mapping = aes(x = displ, y = hwy, color= drv)) + 
  geom_smooth(mapping = aes(linetype = drv), method = 'loess') +
  geom_point()
# default_example

if(save == T){ 
  ggsave(here::here("plots", "default_example.pdf"), default_example, 
...
samples_avg <- samples %>%
  mutate(
    rough_geodist = cut(geodist_miles, breaks = c(0,1,2,3,4,5,6,7,8), labels = FALSE), # bin by mile 
...
ggplot(data = samples_avg, mapping = aes(x = rough_geodist, y = avg_drop, group = 1)) +
  geom_point() +
  geom_line() +
  facet_wrap(vars(City), ncol = 5) + 
  scale_y_continuous(breaks = seq(0,-15) ) + 
  scale_x_continuous(breaks = seq(0,8), minor_breaks = NULL) + 
  labs(
    title = "The average drop in position",
    subtitle = "Drops dramatically in the first mile and quickly stabilizes. Distance are binned by mile."
  ) +
  xlab("Distance in Miles to the location of the law firm") +
  ylab("Drop in the ranking") 
...
Let's have a look at all the samples in a city:  
...
ggplot(data = samples_avg_all, mapping = aes(x = rough_geodist, y = avg_drop, group = Sample)) +
  ...
  labs(
    title = "The average drop in position",
    subtitle = "Whereas most of the law firms see a drop in the first mile, the magnitude of the drop varies greatly. The average is in black."
  ) +
  xlab("Distance in Miles to the location of the law firm") +
  ylab("Drop in the ranking") 
