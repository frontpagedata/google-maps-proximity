...
* conclude
* Check spelling  
* Order cities by size and not alphabetical order
* Check plot design
* add figure number 
...
We start by visualizing the average rank at each mile from the center location for each law firm in each cities. The average rank across all law firms is shown in pink. It is computed with a rank of 21 for the firms out of the top 20 and for which the rank is not recorded anymore by Local Falcon.   
...
```{r, fig.align='center', fig.height=9, out.width='100%'}
...
if(save == T){ 
  ...
  ggsave(here::here("plots",
                    "average_rank_all_samples-10cities_20samples.pdf"),
         plot_to_save, 
...
In order to better compare the law firms, we can visualize their drop from their initial position.  


```{r fig.align='center', fig.height=11, out.width='100%'}
...
if(save == T){ 
  ...
  ggsave(
    here::here("plots",
               "average_drop_all_samples-10cities_20samples.pdf"),
    plot_to_save, 
    width = 15, height = 8, device = cairo_pdf)
}
...
In order to better identify and compare the drop, we focus on the average drop only. __This figure shows the average drop in ranking in relation to the distance from the location of a firm__. It summarizes and simplifies the figures above. Please note that the average are computed with an imputed value of "21" for the ranks out of the top 20.   
...
if(save == T){ 
  ...
  ggsave(here::here("plots",
                    "average_drop_by_city-10cities_20samples.pdf"),
         plot_to_save, 
         width = 15, 
         height = 8, 
         device = cairo_pdf)
...
avg_by_city_and_firm %>% 
  ...
  select(`Distance to Firm` = rough_geodist, 
         `Average Cumulative Drop` = average_drop) %>%
  kable()
...
Google Maps shows 20 results on the first search page and Local Falcon does not collect the ranking above the top 20. We saw above that the ranking was dropping fast in the first mile, but also that not all the firms where dropping out of the top 20 after 10 miles. And this, in all cities, regardless of their area.  
...
How does the proportion of law firms out of the top 20 changes with distance?  
...
if(save == T){ 
  ...
  ggsave(here::here("plots",
                    "perc_out_of_top20-10cities_20samples.pdf"),
         perc_out_of_top20, 
         width = 15, 
         height = 8, 
         device = cairo_pdf)
...
The percentage are computed on 20 sample law firms for the first 6-7 miles, and then on 10 only for the last miles until the distance of 12 to 14 miles from the location of each firm. 
...
As this analysis included a certain amount of uncertainty, we preferred to proceed in stages and offer Rankings.io preliminary analyses with the 10 largest cities.  
...
1. The ranking drops dramatically in the first mile; in all cities. On average, the drop in ranking in the first mile is of -7.4 positions.  
2. The drop in ranking varies greatly between law firms. Some top-ranking firms do not even see a drop in the 10 mile radius. This means that there is probably no distance which would guarantee that all of the law firms in a given city drop out of the top 20.  
...
4. The percentage of law firms which dropped out of the top 20 by mile is varying between cities. For instance, it jumps to almost 40% in the first mile in Atlanta or Philadelphia, but never increases much more. On the contrary, It reaches almost 80% in Boston or Washington DC.     

All these observations might have an impact the current Rankings.io offer of a 5-mile geographic radius around a client’s headquarters to take more than one client in larger cities.   

## Next Steps

Depending on the interests of Rankings.io, this analysis could be:  
* Published as such. A narrative around this preliminary analysis will be proposed to Rankings.io.
* Extended to integrate more cities than the top 10.  
* Extended to collect more samples for the top 10 cities, in order to refine the measurements.   
