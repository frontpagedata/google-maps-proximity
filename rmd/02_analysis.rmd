...
avg_by_firm <- samples %>%
  mutate(
    unique_sample = paste(City, Sample, collapse = "_"),
    rough_geodist = cut(geodist_miles, breaks = seq(0, max(samples$geodist_miles)+1), labels = FALSE), # bin by mile 
    rough_geodist = ifelse(is.na(rough_geodist), 0, rough_geodist) # the bin 0 is 0 instead of NA
  ) %>%
...
We start by illustrating the fit with all the samples in all cities together. In pink, we see the average drop in ranking regardless of the city:  

````{r fig.align='center', fig.height=6, out.width='100%'}

avg_drop_total <- avg_by_city_and_firm %>%
  group_by(rough_geodist) %>%
  summarise(avg_drop = mean(avg_drop))

avg_drop_overall <- ggplot(data = avg_by_city_and_firm, 
                           mapping = aes(x = rough_geodist, 
                                         y = avg_drop,
                                         group = unique_sample)) +
  geom_line(alpha = 0.2) +
