...
In order to better show to the readers what the data represent, we will try to draw heat maps of the data. In other words, try to reproduce the grid. We need to save the data in a different format for this.  

```{r}

all_files <- list.files(path = here::here("raw_data/raw_scans"))
counter = 1
all_samples_grid_list = list() # to store the results
for (file in all_files){
  sample_name = stringr::str_extract(file, '.*(?=\\.csv)')
  print(sample_name)
  # read sheet
  ...
  sample <- sample %>%
    ...
    select(city_sample, Latitude, Longitude, rank = `Your Rank`)

  all_samples_grid_list[[counter]] <- sample
  counter = counter + 1
}

all_samples_on_grid = do.call(rbind, all_samples_grid_list) %>%
  separate(city_sample, into = c("City", "Sample"), sep = "_", remove = FALSE)

# we save a tidy tibble in long format with all the samples on a grid for drawing heat maps
rio::export(all_samples_on_grid, here::here("proc_data", "full_grid_50cities.csv"))
rio::export(all_samples_on_grid, here::here("proc_data", "full_grid_50cities.rds"))
