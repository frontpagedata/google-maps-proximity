...
for (file in all_files){
  ...
  sample <- sample %>% 
    mutate(
      geodist = as.numeric(round(geodist::geodist_vec(Latitude, Longitude, origin_lat, origin_lon)/1000, 1)), # /1000 for km
      rank = case_when(
        `Your Rank` == "20+" ~ 21,
    ...
    select(city_sample, rank, geodist, Latitude, Longitude)
